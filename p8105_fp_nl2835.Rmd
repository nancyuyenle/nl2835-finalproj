...
visualization
```{r rat count over time by borough}
...
```{r plotly histogram for total_refuse}
tr_ggplot = 
  merged |> 
...
```{r plotly histogram for total_paper}
tp_ggplot = 
  merged |> 
...
```{r plotly barchart for total_refuse}
merged |> 
  mutate(borough = fct_reorder(borough, total_refuse)) |> 
